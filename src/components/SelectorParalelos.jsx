...
function SelectorParalelos({ codigoMateria, materiasParalelos, onConfirmar, nombreMateria }) {
    ...
    const handleNext = () => {
        setStartIndex(prev => Math.min(teoricos.length - PARALELOS_POR_PAGINA, prev + PARALELOS_POR_PAGINA));
    };
    ...
    const handleNextPractico = () => {
        setStartIndexPractico(prev => Math.min(practicos.length - PARALELOS_POR_PAGINA, prev + PARALELOS_POR_PAGINA));
    };
                ...
                if (nuevo.day === existente.day) {
                    ...
                    if (
                        ...
                        return {
                            ...
                            dia: nuevo.day,
                            hora: `${nuevo.startTime.slice(0, 5)} - ${nuevo.endTime.slice(0, 5)}`
                        };
                    }
                }
            }
            ...
            if (evento.day === diaExamen) {
                ...
                if (
                    ...
                    return {
                        hayConflicto: true,
                        dia: diaExamen,
                        hora: `${horaInicioExamen.slice(0, 5)} - ${horaFinExamen.slice(0, 5)}`
                    };
                }
            }
        }
    ...
    const confirmarSeleccion = () => {
        ...
        const otrosEventos = eventosActuales.filter(ev => !ev.title.startsWith(codigoMateria));

        // Verificar conflictos con los eventos existentes
        const conflictosTeoricos = hayConflictoHorario(eventosTeorico, otrosEventos);
        ...
        if (eventosPractico.length > 0) {
            const conflictosPracticos = hayConflictoHorario(eventosPractico, otrosEventos);
        ...
        const nuevosEventos = [...otrosEventos, ...eventosTeorico, ...eventosPractico].map((ev, i) => ({
            ...ev,
            id: Date.now() + i,
        }));
    ...
    const paralelosVisibles = teoricos.slice(startIndex, startIndex + PARALELOS_POR_PAGINA);
    const paralelosPracticosVisibles = practicos.slice(startIndexPractico, startIndexPractico + PARALELOS_POR_PAGINA);

    return (
        <div className="p-4">
            {/* Alerta de error/éxito */}
            {errorMensaje && (
                <div className={`fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg max-w-md transform transition-all duration-500 ${errorMensaje.tipo === 'error' ? 'bg-red-100 border-l-4 border-red-500' : 'bg-green-100 border-l-4 border-green-500'}`}>
                    <div className="flex items-start">
                        ...
                        <div className="ml-3">
                            <h3 className={`text-sm font-medium ${errorMensaje.tipo === 'error' ? 'text-red-800' : 'text-green-800'}`}>
                                {errorMensaje.titulo}
                            </h3>
                            <div className={`mt-2 text-sm ${errorMensaje.tipo === 'error' ? 'text-red-700' : 'text-green-700'}`}>
                        ...
                        <div className="ml-auto pl-3">
                            <button
                                ...
                                className={`inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 ${errorMensaje.tipo === 'error' ? 'text-red-500 hover:bg-red-200 focus:ring-red-600' : 'text-green-500 hover:bg-green-200 focus:ring-green-600'}`}
                            >
                ...
                {teoricos.length > PARALELOS_POR_PAGINA && (
                    <button
                        ...
                        className={`
                            ...
                            ${startIndex === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white hover:shadow-xl'}
                        `}
                    >
                ...
                <div className="overflow-hidden">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 transition-transform duration-500 ease-in-out">
                        {paralelosVisibles.map((paralelo, idx) =>
                            paraleloSeleccionado === null || paraleloSeleccionado === startIndex + idx ? (
                                <div
                                    key={startIndex + idx}
                                    className={`border rounded p-3 cursor-pointer hover:bg-blue-100 ${paraleloSeleccionado === startIndex + idx ? "bg-blue-200" : ""}`}
                                    onClick={() => setParaleloSeleccionado(startIndex + idx)}
                                >
                                    <div className="font-bold text-center mb-2">PAR. {paralelo.Paralelo}</div>
                                    <div className="text-center mb-3">
                                        <span className="inline-flex items-center justify-center">
                                            👤 {paralelo.Profesor}
                                        </span>
                                    </div>
                                    <div className={`grid ${paralelo.horarios.length === 1 ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
                                        {paralelo.horarios.map((h, i) => (
                                            <div key={i} className="text-center">
                                                <div className="font-medium">{h.Dia}</div>
                                                <div>⏱️{h.HoraInicio.slice(0, 5)} - {h.HoraFin.slice(0, 5)}</div>
                                                {paralelo.horarios.length === 1 ? (
                                                    <div className="text-sm text-gray-600">📍{h.Aula}</div>
                                                ) : (
                                    ...
                                    {paralelo.horarios.length > 1 && paralelo.horarios[0].Aula === paralelo.horarios[1].Aula && (
                                        <div className="text-sm text-gray-600 text-center mt-2">📍{paralelo.horarios[0].Aula}</div>
                                    )}
                                </div>
                            ) : null
                        )}
                ...
                {teoricos.length > PARALELOS_POR_PAGINA && (
                    <button
                        ...
                        className={`
                            ...
                            flex items-center justify-center
                            transition-all duration-300
                            ${startIndex + PARALELOS_POR_PAGINA >= teoricos.length ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white hover:shadow-xl'}
                        `}
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {paraleloSeleccionado !== null && practicos.length > 0 && (
                <>
                    ...
                    <div className="relative">
                        {/* Flecha izquierda */}
                        {practicos.length > PARALELOS_POR_PAGINA && paraleloPractico === null && (
                            <button
                                ...
                                className={`
                                    ...
                                    flex items-center justify-center
                                    transition-all duration-300
                                    ${startIndexPractico === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white hover:shadow-xl'}
                                `}
                            >
                        ...
                        <div className="overflow-hidden">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 transition-transform duration-500 ease-in-out">
                                {paralelosPracticosVisibles.map((paralelo, idx) =>
                                    paraleloPractico === null || paraleloPractico === startIndexPractico + idx ? (
                                        <div
                                            key={startIndexPractico + idx}
                                            className={`border rounded p-3 cursor-pointer hover:bg-green-100 ${paraleloPractico === startIndexPractico + idx ? "bg-green-200" : ""}`}
                                            onClick={() => setParaleloPractico(startIndexPractico + idx)}
                                        >
                                            <div className="font-bold text-center mb-2">PAR. {paralelo.Paralelo}</div>
                                            <div className="text-center mb-3">
                                                <span className="inline-flex items-center justify-center">
                        ...
                        {practicos.length > PARALELOS_POR_PAGINA && paraleloPractico === null && (
                            <button
                                ...
                                disabled={startIndexPractico + PARALELOS_POR_PAGINA >= practicos.length}
                                className={`
                                    absolute right-0 top-1/2 -translate-y-1/2 z-10
                                    w-10 h-10 rounded-full bg-white/80 shadow-lg
                                    flex items-center justify-center
                                    transition-all duration-300
                                    ${startIndexPractico + PARALELOS_POR_PAGINA >= practicos.length ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white hover:shadow-xl'}
                                `}
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
